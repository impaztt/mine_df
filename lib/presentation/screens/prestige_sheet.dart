...
class PrestigeSheet extends ConsumerWidget {
  const PrestigeSheet({super.key});
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    ...kPrestigeNodes.map((def) {
                      ...
                      final canBuy =
                          !atMax && state.stardust >= cost.ceil();
                      return Padding(
                        ...
                        child: UpgradeCard(
                          ...
                          hint: atMax
                              ? '최대 레벨'
                              : '비용 곡선 ×${def.growthRate.toStringAsFixed(2)} (캡 Lv.${def.maxLevel})',
                          levelBadge: atMax ? 'MAX' : 'Lv.$lv',
                          buttonLabel: atMax ? '최대' : '강화',
                          buttonTimes: 1,
                          // 별의 결정은 코인 아이콘 대신 별로 표시하고 싶지만
                          // UpgradeCard는 코인 표시이므로 메시지에 ✨ 강조
                          cost: atMax ? null : cost.toDouble(),
                          enabled: canBuy,
                          onTap: () {
                            final r =
                                game.upgradePrestigeNode(def.id);
